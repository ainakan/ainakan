...
for task in freebsd_x86_64; do
  ...
  for component in gum gumjs core; do
    enter_artifact ainakan_${component}_devkit

    mv dist/lib/ainakan/devkits/$component/* .
    rm -rf build
    tar -cJf "$output_dir/ainakan-$component-devkit-$host.tar.xz" .

    leave_artifact
  done

  for component in server portal inject gadget; do
    enter_artifact ainakan_${component}

    readarray -t files < <(find . -type f)
    n=${#files[@]}
    if [ $n -ne 1 ]; then
      echo "The ainakan-$component artifact should only contain a single file" > /dev/stderr
      exit 1
    fi
    file=${files[0]}
    name=$(basename $file)
    ...
    xz -c -T 0 "$file" > "$output_dir/ainakan-$component-$host$extension"

    leave_artifact
  done

  enter_artifact ainakan_python
  mv dist/wheels/* "$output_dir"
  leave_artifact

  enter_artifact ainakan_node
