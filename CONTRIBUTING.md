...
Ainakan is composed of many sub-projects and its code spans across many
...
```c
static gboolean
ainakan_find_libSystem (const GumModuleDetails * details, gpointer user_data)
{
    ...
}
```
...
```c
    case AINAKAN_BREAKPOINT_CLEANUP:
    {
      task_t self_task;
      gsize page_size;
      AinakanExceptionPortSet * previous_ports;
      ...
      for (i = 0; i != AINAKAN_MAX_BREAKPOINTS; i++)
        ainakan_spawn_instance_unset_nth_breakpoint (self, i);
    }
```
...
```c
static csh
ainakan_create_capstone (GumCpuType cpu_type, GumAddress start)
{
  csh capstone;
  cs_err err;
...
```c
guint
_ainakan_darwin_helper_backend_demonitor_and_clone_injectee_state (AinakanDarwinHelperBackend * self, void * raw_instance)
{
  AinakanInjectInstance * instance = raw_instance;
  AinakanInjectInstance * clone;
...
```c
static gboolean ainakan_find_libsystem (const GumModuleDetails * details, gpointer user_data)
{
  ...
}
```
...
```c
    ...
    else
    {
      ...
      ainakan_error = g_error_new (
          AINAKAN_ERROR,
          AINAKAN_ERROR_NOT_SUPPORTED,
...
```c
  if (_ainakan_get_springboard_api ()->fbs != NULL)
    ainakan_darwin_helper_backend_launch_using_fbs (identifier_value, url_value, options, aux_options, on_complete,
        on_complete_target);
  else
    ainakan_darwin_helper_backend_launch_using_sbs (identifier_value, url_value, options, aux_options, on_complete,
        on_complete_target);
```
...
without having to first `ainakan-compile` the code.
...
# Generic rules in ainakan-gum

This set of rules apply to all code in ainakan-gum, regardless of the
language.
...
# Rules for C code in ainakan-gum
...
# Generic rules in ainakan-core

This set of rules apply to all code in ainakan-core, regardless of the
language.
...
# Rules for C code in ainakan-core

This set of rules apply to C code belonging to ainakan-core.

## Function definitions

### Rule

Function names should be “namespaced” by having a `ainakan_` prefix,
even if static.
